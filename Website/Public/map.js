...
function selectState(s){
	...
	hideAllStates()
	showState(str)
	showElectionDates(str)

	history.pushState({state: true}, null, "index.html")
...
function toRelativeOrAbsoluteProbabilities(){
	var buttonRelative = document.getElementById("relative")
	if(relative){
		buttonRelative.innerHTML = "Relative predictions"
	} else {
		buttonRelative.innerHTML = "Absolute predictions"
	}
	relative = !relative
	drawCountry()
}
...
function hideAllStates(){
	states.forEach((x)=>{
		x.node().setAttribute("fill", "grey")
	})
	behaviour = 1
}

function showAllStates(){
	states.forEach((x)=>{x.node().setAttribute("fill", "black")})
	behaviour = 0
}

function showState(str){
	states.forEach((x)=>{
		if(str == x.info.data("id")){
			x.node().setAttribute("fill", "black")
		}
	})
}
...
function showElectionDates(str){
	...
	for(var i = 0; i < electionDates.length; ++i){
		var opt = document.createElement("option")
		opt.textContent = electionDates[i]
